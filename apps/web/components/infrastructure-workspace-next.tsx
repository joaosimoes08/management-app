...
export function InfrastructureWorkspace() {
  ...
  const load = async (id = siteId) => {
    ...
    try {
      ...
      if(requestedDevice){setDevice(requestedDevice);setTab('interfaces');const interfaceData=await apiFetch(`/api/v1/interfaces?deviceId=${requestedDevice.id}`);const ordered=(interfaceData??[]).sort(naturalInterfaceCompare);setInterfaces(ordered);setSelectedInterface(ordered.find((item:Any)=>item.id===query.get('interfaceId'))??null)}
