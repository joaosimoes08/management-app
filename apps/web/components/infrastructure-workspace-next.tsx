...
export function InfrastructureWorkspace() {
  ...
  const chooseUpload = (event: Any) => { const file = event.target.files?.[0]; event.target.value = ''; if (!file) return; setPendingUpload(file); setAssetName((current: string) => current.trim() || file.name); };
