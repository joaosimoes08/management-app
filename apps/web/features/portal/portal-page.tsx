...
import type { ApplicationLink } from './api';
import { checkApplicationLink, createApplicationLink, listApplicationLinks, removeApplicationLink, updateApplicationLink } from './api';
...
export default function PortalPage() {
  ...
  const { data: links = [], isLoading: loading, refetch } = useQuery({ queryKey: ['portal', 'links', admin], queryFn: () => listApplicationLinks(admin) });
  const loadLinks = () => void refetch();
  const visibleLinks = useMemo(() => { const query = searchQuery.trim().toLowerCase(); return query ? links.filter((link) => `${link.name} ${link.category} ${link.description ?? ''}`.toLowerCase().includes(query)) : links; }, [links, searchQuery]);
  const categories = useMemo(() => Array.from(new Set(visibleLinks.map((link) => link.category))), [visibleLinks]);
  ...
  async function saveLink(event: FormEvent<HTMLFormElement>) {
    ...
    try { if (form.id) await updateApplicationLink(form.id, payload); else await createApplicationLink(payload); setShowForm(false); setForm(emptyForm); await queryClient.invalidateQueries({ queryKey: ['portal', 'links'] }); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Não foi possível guardar a aplicação.'); }
    finally { setSaving(false); }
  }

  async function removeLink(id: string) { if (!window.confirm('Remover esta aplicação do portal?')) return; try { await removeApplicationLink(id); await queryClient.invalidateQueries({ queryKey: ['portal', 'links'] }); } catch { setError('Não foi possível remover a aplicação.'); } }
  async function checkLink(id: string) { setChecking(id); try { const updated = await checkApplicationLink(id); queryClient.setQueryData<ApplicationLink[]>(['portal', 'links', admin], (current) => (current ?? []).map((link) => link.id === id ? updated : link)); } catch { setError('Não foi possível verificar a disponibilidade.'); } finally { setChecking(undefined); } }

  return <AppShell section="Portal interno" search={{ value: searchQuery, onChange: setSearchQuery, placeholder: 'Pesquisar aplicações…' }} actions={admin ? <button className="topbar-action secondary-button" onClick={() => void loadLinks()}><Activity size={14} /> Atualizar</button> : undefined}><main className="portal-page">
    <div className="portal-topline"><div><span className="section-kicker">COCIBER / APLICAÇÕES</span><h1>Aplicações chave</h1></div>{admin && <div className="portal-actions"><button className="primary-button" onClick={openCreate}><Plus size={15} /> Nova aplicação</button></div>}</div>
