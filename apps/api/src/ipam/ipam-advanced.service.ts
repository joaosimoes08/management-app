...
import { CalculatorDto, CreateIpamGroupDto, CreateNatRuleDto, CreateSubnetDto, CreateVrfDto, UpdateIpamPermissionDto, UpdateNatRuleDto, UpdateSubnetScanDto, UpdateVrfDto } from './dto';
...
@Injectable()
export class IpamAdvancedService {
  ...
  async getSubnet(id: string) { const item = await this.prisma.subnet.findUnique({ where: { id }, include: { site: true, vlan: true, vrf: true, parentSubnet: true, childSubnets: true, discoverySchedule: true } }); if (!item) throw new NotFoundException('Subnet não encontrada'); return item; }
  async subnetUsage(id: string) { const subnet = await this.getSubnet(id); const counts = await this.prisma.ipAddress.groupBy({ by: ['state'], where: { subnetId: id }, _count: { _all: true } }); const byState = Object.fromEntries(counts.map((item) => [item.state, item._count._all])); const info = cidrInfo(subnet.cidr); const total = info.version === 4 ? info.broadcast - info.network + 1 : info.total; const occupied = (byState.OCCUPIED ?? 0) + (byState.UNKNOWN ?? 0); return { subnet, version: info.version, total, theoreticalCapacity: total, known: Object.values(byState).reduce((a, b) => a + b, 0), occupied, free: info.version === 4 && typeof total === 'number' ? Math.max(0, total - Object.values(byState).reduce((a, b) => a + b, 0)) : null, byState, utilizationPercent: typeof total === 'number' && total > 0 ? Math.round((occupied / total) * 10000) / 100 : null }; }
  async subnetTree(id: string) { const subnet = await this.getSubnet(id); return { root: subnet, children: subnet.childSubnets }; }
  ...
  async updateSubnetScan(id: string, dto: UpdateSubnetScanDto, user: AuthenticatedUser) { const subnet = await this.getSubnet(id); const methods = [...new Set(dto.methods)]; if (!methods.length || (methods.includes('TCP') && !dto.tcpPorts?.length)) throw new BadRequestException('Configuração de scanning inválida'); const next = dto.enabled ? new Date(Date.now() + 12 * 60 * 60 * 1000) : null; const item = await this.prisma.subnet.update({ where: { id }, data: { scanEnabled: dto.enabled, scanIntervalHours: 12, scanMethods: methods, scanTcpPorts: dto.tcpPorts ?? [], reverseDnsEnabled: dto.reverseDns ?? true, nextScanAt: next } }); if (dto.enabled) await this.queue.upsertJobScheduler(`subnet-${id}`, { every: 12 * 60 * 60 * 1000 }, { name: 'scheduled-subnet-scan', data: { subnetId: id } }); else await this.queue.removeJobScheduler(`subnet-${id}`); await this.log(user, 'SUBNET_SCAN_CONFIG_UPDATED', 'Subnet', id, dto); return item; }
  async scanSubnet(id: string, user: AuthenticatedUser) { const subnet = await this.getSubnet(id); const methods = Array.isArray(subnet.scanMethods) ? subnet.scanMethods.filter((value): value is string => typeof value === 'string') : ['ICMP']; const tcpPorts = Array.isArray(subnet.scanTcpPorts) ? subnet.scanTcpPorts.filter((value): value is number => typeof value === 'number') : []; const job = await this.prisma.discoveryJob.create({ data: { name: `IPAM check · ${subnet.cidr}`, subnetId: id, methods, tcpPorts } }); await this.queue.add('scan-subnet', { jobId: job.id, cidr: subnet.cidr, methods, tcpPorts }, { jobId: job.id, removeOnComplete: 100, removeOnFail: 100 }); await this.log(user, 'SUBNET_SCAN_QUEUED', 'Subnet', id, { jobId: job.id }); return job; }
  async getIp(id: string) { const item = await this.prisma.ipAddress.findUnique({ where: { id }, include: { subnet: { include: { vlan: true, vrf: true, site: true } }, host: true, device: true, interface: true } }); if (!item) throw new NotFoundException('IP não encontrado'); return item; }
  async checkIp(id: string, user: AuthenticatedUser) { const ip = await this.getIp(id); const started = Date.now(); let reachable = false; if (net.isIP(ip.address) === 4) { const socket = await new Promise<boolean>((resolve) => { const client = net.createConnection({ host: ip.address, port: 443, timeout: 800 }); client.once('connect', () => { client.destroy(); resolve(true); }); client.once('error', () => resolve(false)); client.once('timeout', () => { client.destroy(); resolve(false); }); }); reachable = socket; } const updated = await this.prisma.ipAddress.update({ where: { id }, data: { lastCheckAt: new Date(), lastCheckMethod: 'TCP', responseMs: Date.now() - started, observedState: reachable ? 'REACHABLE' : 'UNREACHABLE', icmpReachable: null } }); await this.log(user, 'IP_CHECKED', 'IpAddress', id, { reachable }); return updated; }
  async listVrfs(siteId?: string, search?: string) { return this.prisma.vrf.findMany({ where: { ...(siteId ? { siteId } : {}), ...(search ? { name: { contains: search, mode: 'insensitive' } } : {}) }, include: { _count: { select: { subnets: true, natRules: true } } }, orderBy: { name: 'asc' } }); }
  async getVrf(id: string) { const item = await this.prisma.vrf.findUnique({ where: { id }, include: { site: true, subnets: true, natRules: true } }); if (!item) throw new NotFoundException('VRF não encontrado'); return item; }
  async createVrf(dto: CreateVrfDto, user: AuthenticatedUser) { const item = await this.prisma.vrf.create({ data: { ...dto, status: dto.status ?? 'ACTIVE' } }); await this.log(user, 'VRF_CREATED', 'Vrf', item.id); return item; }
  async updateVrf(id: string, dto: UpdateVrfDto, user: AuthenticatedUser) { const item = await this.prisma.vrf.update({ where: { id }, data: dto }).catch(() => { throw new NotFoundException('VRF não encontrado'); }); await this.log(user, 'VRF_UPDATED', 'Vrf', id); return item; }
  async deleteVrf(id: string, user: AuthenticatedUser) { const item = await this.getVrf(id); if (item.subnets.length) throw new ConflictException('Não é possível remover um VRF com subnets'); await this.prisma.vrf.delete({ where: { id } }); await this.log(user, 'VRF_DELETED', 'Vrf', id); return { success: true }; }
  async listNatRules(siteId?: string, vrfId?: string, type?: string, search?: string) { return this.prisma.natRule.findMany({ where: { ...(siteId ? { siteId } : {}), ...(vrfId ? { vrfId } : {}), ...(type ? { type: type as never } : {}), ...(search ? { OR: [{ name: { contains: search, mode: 'insensitive' } }, { sourceAddress: { contains: search } }, { translatedAddress: { contains: search } }] } : {}) }, include: { sourceSubnet: true, translatedSubnet: true, sourceIp: true, translatedIp: true, device: true, vrf: true }, orderBy: { name: 'asc' } }); }
  async getNatRule(id: string) { const item = await this.prisma.natRule.findUnique({ where: { id }, include: { sourceSubnet: true, translatedSubnet: true, sourceIp: true, translatedIp: true, device: true, vrf: true } }); if (!item) throw new NotFoundException('Regra NAT não encontrada'); return item; }
  async createNatRule(dto: CreateNatRuleDto, user: AuthenticatedUser) { const item = await this.prisma.natRule.create({ data: { ...dto, type: dto.type as never, enabled: dto.enabled ?? true } }); await this.log(user, 'NAT_RULE_CREATED', 'NatRule', item.id); return item; }
  async updateNatRule(id: string, dto: UpdateNatRuleDto, user: AuthenticatedUser) { const item = await this.prisma.natRule.update({ where: { id }, data: { ...dto, type: dto.type as never } }).catch(() => { throw new NotFoundException('Regra NAT não encontrada'); }); await this.log(user, 'NAT_RULE_UPDATED', 'NatRule', id); return item; }
  async deleteNatRule(id: string, user: AuthenticatedUser) { await this.getNatRule(id); await this.prisma.natRule.delete({ where: { id } }); await this.log(user, 'NAT_RULE_DELETED', 'NatRule', id); return { success: true }; }
  ...
  async createIpamPermission(dto: { groupId: string; scopeType: string; scopeId: string; permission: string }, user: AuthenticatedUser) { const item = await this.prisma.ipamPermission.create({ data: dto }); await this.log(user, 'IPAM_PERMISSION_CREATED', 'IpamPermission', item.id); return item; }
  async updateIpamPermission(id: string, dto: UpdateIpamPermissionDto, user: AuthenticatedUser) { const item = await this.prisma.ipamPermission.update({ where: { id }, data: dto }).catch(() => { throw new NotFoundException('Permissão não encontrada'); }); await this.log(user, 'IPAM_PERMISSION_UPDATED', 'IpamPermission', id); return item; }
